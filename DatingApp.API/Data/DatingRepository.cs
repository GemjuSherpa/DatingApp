...
namespace DatingApp.API.Data
{
  public class DatingRepository : IDatingRepository
  {
    ...
    public async Task<PagedList<User>> GetUsers(UserParams userParams)
    {
      ...
      users = users.Where(u => u.Id != userParams.UserId); //exclude logged in user
      users = users.Where(u => u.gender == userParams.Gender); //filtering by gender
      ...
      //Sorting by last active and created date.
    ...
    //GetUserLikes=> get the list of liked user
    ...
    {
      return await _context.Messages.FirstOrDefaultAsync(m => m.MessageId == id);
    }
    ...
    public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipientId)
    {
      ...
      .Include(u => u.Sender).ThenInclude(p => p.Photos).Include(u => u.Recipient).ThenInclude(p => p.Photos)
